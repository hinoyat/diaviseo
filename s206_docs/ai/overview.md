...
- 피드백은 MongoDB + MySQL(feedback_tb)에 저장
...
| 기능 | 모듈 |
|------|------|
| 피드백 저장 | `insert_feedback()` in MySQL |
...
- 현 모델은 **수치 계산 또는 다단계 추론**  
  (예: BMI → 칼로리 계산 → 식단 구성) 과정에서 오류 빈도가 높은 편입니다.
...
- `GPT-4o`, `Command R`, `Claude 3`, `LLaMA 3` 등  
  고성능 언어모델의 도입을 통해 추론 정확도 및 대응 범위를 확장할 수 있습니다.

---

**이 문서는 프로젝트 종료 시점을 기준으로 정리된 시스템 구성 문서입니다. AI 챗봇 서비스의 전체적인 백엔드 구조와 주요 설계 내용을 정리한 문서이며, 세부 모듈의 구현 및 호출 방식은 각 기능별 개발자 문서를 참고해 주세요.**
