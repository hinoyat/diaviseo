...
def get_user_info_for_feedback(health_db: Session, user_db: Session, user_id: int) -> dict:
  ...
  remaining_calorie = calculate_remaining_calories(health_db, user_db, user_id)
...
def calculate_remaining_calories(health_db: Session, user_db: Session, user_id: int):
  intake = float(get_today_calories_intake(health_db, user_id, meal_date=datetime.date.today()))
  print(f"User {user_id}의 오늘 칼로리 섭취량: {intake} kcal")

  burned = float(get_today_calories_burned(health_db, user_id))
  print(f"User {user_id}의 오늘 소모한 칼로리: {burned} kcal")
  bmr = get_bmr(user_db, health_db, user_id)
  remaining = intake - bmr - burned
  return remaining  # 이 값이 양수면 '더 소모해야 할 칼로리'
